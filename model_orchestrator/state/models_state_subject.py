    ...
    def notify_runner_state_changed(self, model_run: ModelRun, previous_state: ModelRun.RunnerStatus | None, new_state: ModelRun.RunnerStatus):
        ...
        for observer in self.observers:
            try:
                ...
                get_logger().error(f"{NEW_RELIC_ALERT_FLAG} Error notifying observer {type(observer)} about model run state change of model uid:{model_run.id}, model_id:{model_run.model_id}", exc_info=e)
    ...
    def notify_build_state_changed(self, model_run: ModelRun, previous_state: ModelRun.BuilderStatus | None, new_state: ModelRun.BuilderStatus):
        ...
        for observer in self.observers:
            try:
                ...
                get_logger().error(f"{NEW_RELIC_ALERT_FLAG} Error notifying observer {type(observer)} about model build state change of model uid:{model_run.id}, model_id:{model_run.model_id}", exc_info=e)
    ...
    def notify_failure(self, model_run: ModelRun, failure):
        for observer in self.observers:
            try:
                ...
                get_logger().error(f"{NEW_RELIC_ALERT_FLAG} Error notifying observer {type(observer)} about model failure of model uid:{model_run.id}, model_id:{model_run.model_id}", exc_info=e)
