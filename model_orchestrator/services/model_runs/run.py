...
class _RunService:
    ...
    def update_models_info(self, models=None):
        ...
        try:
            ...
            get_logger().error("API provider error: failed to load models augmented info", exc_info=e)
    ...
    def update_runner_states(self):
        ...
        try:
            ...
            get_logger().error("Cloud provider error: failed to load models run status", exc_info=e)
            ...
            for model, (updated_status, ip, port) in models_states.items():
                ...
                if updated_status != previous_status:
                    ...
                    if updated_status == ModelRun.RunnerStatus.RUNNING:
                        ...
                        replacing_models = self.cluster.get_models_by_model_id_and_desired_status(model.model_id, [ModelRun.DesiredStatus.REPLACING])
                        for replacing_model in replacing_models:
                            get_logger().info(f'Model {model.model_id} is now running, stopping replaced model {replacing_model.id}')
                            try:
                                self.stop_model(replacing_model)
